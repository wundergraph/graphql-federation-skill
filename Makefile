...
verify:
	@echo "Verifying $(SKILL_FILE)..."
	@test -s $(SKILL_FILE) || (echo "Error: $(SKILL_FILE) is empty or missing" && exit 1)
	@unzip -t $(SKILL_FILE) > /dev/null || (echo "Error: $(SKILL_FILE) is not a valid zip file" && exit 1)
